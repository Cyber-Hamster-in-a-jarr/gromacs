...
void gmx::LegacySimulator::do_extended_rerun()
{
    ...
    if (ir->bExpanded)
    {
        gmx_fatal(FARGS, "Expanded ensemble not supported by ererun.");
    }
    if (ir->bDoAwh)
    {
        gmx_fatal(FARGS, "AWH not supported by ererun.");
    }
    if (ir->bIMD)
    {
        gmx_fatal(FARGS, "Interactive MD not supported by ererun.");
    }
    ...
    if (isMultiSim(ms_))
    {
        gmx_fatal(FARGS, "Multiple simulations not supported by ererun.");
    }
    ...
    ForceBuffers     f(simulationWork.useMts,
                       simulationWork.useGpuFBufferOpsWhenAllowed//(simulationWork.useGpuFBufferOpsWhenAllowed || useGpuForUpdate)
    ...
    auto awh = nullptr; /* prepareAwhModule(fpLog_,
                                 *ir,
                                 stateGlobal_,
                                 cr_,
                                 ms_,
    ...
    if (PAR(cr_))
    {
        rerun_parallel_comm(cr_, &rerun_fr, &bLastStep);
    }
    ...
    while (!bLastStep)
    {
            ...
            if (haveDDAtomOrdering(*cr_))
            {
                ...
                dd_partition_system(fpLog_,
                                    ...
                                    *ir,
                                    ...
                                    nrnb_,
                                    wallCycleCounters_,
                                    mdrunOptions_.verbose /*&& !bPMETunePrinting*/);
                upd.updateAfterPartition(state_->numAtoms(), md->cFREEZE, md->cTC, md->cACC);
                fr_->longRangeNonbondeds->updateAfterPartition(*md);
            }
        // }
        ...
        // {
            ...
            do_md_trajectory_writing(fpLog_,
                                     ...
                                     ekind_,
                                     f.view().force(),
                                     false, /*checkpointHandler->isCheckpointingStep(),*/
                                     bRerunMD,
                                     bLastStep,
                                     mdrunOptions_.writeConfout,
                                     ekindataState);
        ...
        if (isMainSimMainRank(ms_, MAIN(cr_)) && (mdrunOptions_.verbose || gmx_got_usr_signal()) /* && !bPMETunePrinting */)
        {
            if (shellfc)
            {
                fprintf(stderr, "\n");
        ...
        if (MAIN(cr_))
        {
            ...
            bLastStep = !read_next_frame(oenv_, status, &rerun_fr);
        }
        if (PAR(cr_))
        {
            rerun_parallel_comm(cr_, &rerun_fr, &bLastStep);
